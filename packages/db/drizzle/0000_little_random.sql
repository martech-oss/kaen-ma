...
CREATE TABLE `broadcasts` (
	...
	FOREIGN KEY (`template_version_id`) REFERENCES `email_template_versions`(`id`) ON UPDATE no action ON DELETE restrict,
...
CREATE UNIQUE INDEX `campaign_enrollment_source_unique` ON `campaign_enrollments` (`workspace_id`,`campaign_version_id`,`contact_id`,`source_event_id`);--> statement-breakpoint
...
CREATE UNIQUE INDEX `companies_workspace_domain_unique` ON `companies` (`workspace_id`,`domain`);--> statement-breakpoint
...
CREATE UNIQUE INDEX `contacts_workspace_visitor_unique` ON `contacts` (`workspace_id`,`visitor_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `contacts_workspace_external_unique` ON `contacts` (`workspace_id`,`external_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `contacts_workspace_email_unique` ON `contacts` (`workspace_id`,`email`);--> statement-breakpoint
...
CREATE TABLE `deliveries` (
	...
	FOREIGN KEY (`topic_id`) REFERENCES `subscription_topics`(`id`) ON UPDATE no action ON DELETE set null,
	FOREIGN KEY (`template_version_id`) REFERENCES `email_template_versions`(`id`) ON UPDATE no action ON DELETE set null,
	...
	CONSTRAINT "deliveries_provider_check" CHECK("deliveries"."provider" IN ('cloudflare', 'postmark', 'resend', 'webhook')),
...
--> statement-breakpoint
CREATE UNIQUE INDEX `email_versions_workspace_template_version_unique` ON `email_template_versions` (`workspace_id`,`template_id`,`version`);--> statement-breakpoint
CREATE TABLE `email_templates` (
	...
	CONSTRAINT "email_templates_status_check" CHECK("email_templates"."status" IN ('draft', 'published', 'archived'))
);
--> statement-breakpoint
CREATE INDEX `email_templates_workspace_updated_idx` ON `email_templates` (`workspace_id`,`updated_at`);--> statement-breakpoint
...
CREATE TABLE `provider_configs` (
	...
	`updated_at` text NOT NULL,
	FOREIGN KEY (`workspace_id`) REFERENCES `organization`(`id`) ON UPDATE no action ON DELETE cascade,
	CONSTRAINT "provider_configs_provider_check" CHECK("provider_configs"."provider" IN ('cloudflare', 'postmark', 'resend', 'webhook'))
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX `suppressions_workspace_email_unique` ON `suppressions` (`workspace_id`,`email`);--> statement-breakpoint
CREATE TABLE `tags` (
	`id` text PRIMARY KEY NOT NULL,
	`workspace_id` text NOT NULL,
	`name` text NOT NULL,
